...
function render(){
 scene=api.geometry($('joint').value,{cutaway:$('cutaway').checked});const d=scene.detail,p=d.product;
 ...
 $('title').textContent=d.name;$('product').textContent=p.code+' · '+p.diameter+' × '+p.length+' mm · single-position study';
...
for(const id of ['joint','cutaway'])$(id).onchange=render;
