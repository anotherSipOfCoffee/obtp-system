...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-gl=angle','--use-angle=swiftshader','--enable-webgl']});try{
 ...
 for(const key of ['wall-seam','floor-seam','roof-seam','corner','wall-floor','wall-roof']){
  ...
  const before=await page.evaluate(()=>OBTPDetailView.scene.detail.checks);await page.locator('#view').selectOption('full');assert.deepEqual(await page.evaluate(()=>OBTPDetailView.scene.detail.checks),before);await page.locator('#view').selectOption('cutaway');
 ...
 await page.locator('#view').selectOption('full');await page.screenshot({path:'qa/detail-wall-floor-revised-full.png',fullPage:true});
 await page.locator('#view').selectOption('cutaway');await page.screenshot({path:'qa/detail-wall-floor-revised-cutaway.png',fullPage:true});
 ...
 assert.deepEqual(errors,[]);console.log('PASS: twelve original/revised connection detail views, rejection labels, view-invariant checks, WebGL, cutaway, separation/reset, honest inspection export, mobile layout and System entry link.');
