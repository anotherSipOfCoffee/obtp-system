...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-gl=angle','--use-angle=swiftshader','--enable-webgl']});try{
 ...
 const download=page.waitForEvent('download');await page.locator('#export').click();const d=await download;await d.saveAs('qa/cassette-rhino.json');const geometry=JSON.parse(fs.readFileSync('qa/cassette-rhino.json'));assert.equal(geometry.manufacturingRelease,false);assert(geometry.parts.length>100);assert.equal(geometry.units,'mm');assert.equal(geometry.connectionResearch.manufacturingRelease,false);assert.equal(geometry.connectionResearch.interfaces.length,geometry.interfaces.length);
