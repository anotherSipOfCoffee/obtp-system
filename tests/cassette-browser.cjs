...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-gl=angle','--use-angle=swiftshader','--enable-webgl']});try{
 ...
 assert.equal(await page.locator('aside select').first().getAttribute('id'),'version');assert.equal(await state(()=>OBTPCassetteView.items.length),30);
 ...
 await type('Roofs');await view('assembly');await page.locator('#bays').selectOption('8');assert.equal(await state(()=>OBTPCassetteView.items.length),46);await page.locator('#layer').selectOption('floor');assert.equal(await state(()=>OBTPCassetteView.items.length),8);await page.locator('#layer').selectOption('all');await page.locator('#bays').selectOption('4');await page.locator('#height').selectOption('2100');
