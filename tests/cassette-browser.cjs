...
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-gl=angle','--use-angle=swiftshader','--enable-webgl']});try{
 ...
 assert.equal(await page.locator('#version').inputValue(),'index.html');assert.equal(await page.locator('aside select').first().getAttribute('id'),'version');assert.equal(await page.locator('#revision').inputValue(),'revised');
 assert.equal(await page.locator('canvas').evaluate(c=>c.getContext('webgl').getError()),0);
 for(const type of ['Floors','Roofs','Walls']){await page.locator('[data-type="'+type+'"]').click();assert.equal(await page.evaluate(()=>OBTPCassetteView.items.length),1);assert.equal(await page.locator('#objects button').count()>0,true);}await page.locator('[data-view="assembly"]').click();
 ...
 await page.locator('[data-view="object"]').click();assert.equal(await page.evaluate(()=>OBTPCassetteView.items.length),1);
 fs.mkdirSync('qa',{recursive:true});
 for(const type of ['panel-frame','wall-seam','corner','wall-floor','wall-roof','slab-seam','frame-frame']){
  await page.locator('[data-view="connection"]').click();await page.locator('#joint').selectOption(type);
 ...
 await page.locator('[data-view="assembly"]').click();await page.locator('#bays').selectOption('1');await page.locator('[data-view="connection"]').click();await page.locator('#joint').selectOption('slab-seam');assert.match(await page.locator('#connection-record').textContent(),/no adjacent/);
 ...
 await page.locator('[data-view="assembly"]').click();await page.locator('#bays').selectOption('8');await page.locator('[data-view="connection"]').click();await page.locator('#joint-view').selectOption('detail');
 await page.locator('[data-view="assembly"]').click();await page.locator('#layer').selectOption('floor');assert.equal(await page.evaluate(()=>OBTPCassetteView.items.length),8);
 ...
 await page.locator('#version').selectOption('../index.html');await page.waitForFunction(()=>window.OBTPSystem?.selected==='W-S');assert.equal(await page.locator('aside select').first().getAttribute('id'),'version');await page.locator('#version').selectOption('cassette/index.html');await page.waitForFunction(()=>window.OBTPCassetteView?.scene.connectionRevision==='revised');await page.locator('a[href="research.html"]').click();await page.locator('h1').first().waitFor();assert.deepEqual(errors,[]);console.log('PASS cassette browser controls, export, desktop/mobile, no page errors');
