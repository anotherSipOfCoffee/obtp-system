...
def overlap(a,b):
    return all(min(a['origin'][k]+a['size'][k],b['origin'][k]+b['size'][k])-max(a['origin'][k],b['origin'][k])>1e-7 for k in range(3))
...
class AuthoringTests(unittest.TestCase):
    ...
    def test_six(self):
        ...
        for i in range(6):
            ...
            self.assertEqual(len(s['opening_voids']),3 if i%2 else 2)
