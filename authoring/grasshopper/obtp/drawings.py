...
def derive(scene):
    ...
    views={};cuts=[('plan',2,F+1100),('section-a',0,195+p['sauna_length_steps']*600/2),('section-b',1,W/2)]
    for name,axis,level in cuts:
        ...
        for part in scene['parts']:
            ...
            polygons.append(dict(id=part['id'],points=poly,fill=fill,cut=cut))
        # Put black sectioned walls above projected furniture/floor.
        polygons.sort(key=lambda a:a['fill']=='#111111')
        if name=='plan':
    ...
    polys=[]
    for a in window:
        polys.append(dict(id=a['id'],points=hull([[v[0]-x0,v[2]-z0] for v in vertices(a)]),fill='#eeeeee' if a['material']=='glass' else '#ffffff',cut=False))
    views['window']=dict(axis=1,level_mm=98,polygons=polys,polylines=[],dimensions=[dimension([0,0],[p['window_width'],0],-180),dimension([p['window_width'],0],[p['window_width'],p['door_height']],180)])
    views['window-plan']=dict(axis=2,level_mm=F+1100,polygons=[dict(id=a['id'],points=[[v[0]-x0,v[1]] for v in section(a,2,F+1100)],fill='#eeeeee',cut=True) for a in window],polylines=[],dimensions=[])
    ...
    return dict(schema='obtp-drawings/1',source_geometry_sha256=scene['geometry_sha256'],units='mm',views=views,
