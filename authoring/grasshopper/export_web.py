...
def compile_catalogue(destination,revision):
    ...
    for i,roof,terrace,window in itertools.product(range(6),range(3),[1,2],[600,900,1200]):
        ...
        (target/(key+'-plan.svg')).write_text(svg(scene['drawings']['views']['plan']))
    ...
    with tempfile.TemporaryDirectory() as temp:
        with zipfile.ZipFile(target/'OBTP_Grasshopper_R04.zip','w',zipfile.ZIP_DEFLATED) as z:
            ...
            for i in range(6):
                ...
                key=stem+'-r1-t2-w1200-f0';z.write(target/(key+'.pdf'),'exports/'+stem+'.pdf')
