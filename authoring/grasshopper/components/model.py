...
for module in ['cut_view','seasonal','object_library','suppliers','export','plan_styles','drawings','insulation','envelope']:
...
try:
    ...
    studio_winter_closed=globals().get('studio_winter_closed',True)
    ...
    for _input in ['preset_index','room_depth_steps','sauna_length_steps','hall_length_steps',
                   'foundation_type','roof_type','terrace_steps','window_width','facade_type','system_type','program_type','storage_length_steps','wall_height','partition_depth','door_width','door_height',
                   'sauna_door_offset','bench_depth','bench_height','foot_bench_height']:
        globals()[_input]=float(scalar(globals()[_input]))
    for _input in ['custom','storage','include_foundation','studio_winter_closed']:
        globals()[_input]=bool(scalar(globals()[_input]))
    p=core.parameters(int(preset_index),bool(custom),
        foundation_type=foundation_type,studio_winter_closed=studio_winter_closed,program_type=program_type,system_type=system_type,roof_type=roof_type,terrace_steps=terrace_steps,window_width=window_width,facade_type=facade_type,
    ...
    report_objects='Linked library: {} placed objects (coordination placeholders)'.format(len(scene['cut_view','seasonal','object_library']['instances']))
    m=scene['metrics']
